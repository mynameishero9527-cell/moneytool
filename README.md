...
| 全 A 行情快照（流通市值、换手） | 东方财富，限流时新浪 | 新浪行情中心 `hs_a` 节点，每页 100 只 |
| 概念板块与成分 | 东方财富，限流时新浪 | 新浪约 175 个概念（`gn_` 开头）；已有概念沿用原来源，不混两套 |

新浪来源下，回补线程每天 18 点后给每只股票增量拉取最新几天（每只每 6 小时最多试一次），某天的结果若当时用的是收盘快照，拿到新浪正式值后会自动重算。
